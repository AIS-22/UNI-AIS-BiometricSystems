...
def print_accuracy():
    accuracy = np.load('results/resnet18_gen_spoof_accuracy_results.npy')
    print('accuracy:', accuracy)
def plot_confusion_matrix():
    categories = ['genuine', 'spoofed']

    # load dic from file
    conf_matrix = np.load('results/resnet18_gen_spoof_conf_matrix_results.npy', allow_pickle=True)
    cmap = plt.get_cmap('tab20')

    plt.figure(figsize=(15, 10))
    sn.set(font_scale=1.4)
    sn.heatmap(conf_matrix, vmin=0, vmax=79,
